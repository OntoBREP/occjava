...
/**
 * Useful toolbox. Note that this methods are not Open CASCADE binding and
 * should probably not be used outside of the jCAE project, as compatibility
 * between versions won't be warranted.
 */
public class Utilities {
	...
	private static void dumpTopology(TopoDS_Shape shape, PrintStream out,
			int level) {
		TopoDS_Iterator it = new TopoDS_Iterator(shape);
		char[] dots = new char[level * TAB];
		Arrays.fill(dots, '-');
		String dotss = new String(dots);
		String label = shape.toString().substring(
				"org.jcae.opencascade.jni.TopoDS_".length());
	...
	public static int numberOfShape(TopoDS_Shape shape, TopAbs_ShapeEnum type) {
		int n = 0;
		for (TopExp_Explorer exp = new TopExp_Explorer(shape, type); exp.more(); exp
				.next())
			n++;
		return n;
	}
	...
	public static boolean isShapeInShape(TopoDS_Shape parent, TopoDS_Shape child) {
		for (TopExp_Explorer exp = new TopExp_Explorer(parent,
				child.shapeType()); exp.more(); exp.next()) {
	...
	/**
	 * Read a file guessing the format with the file extension Only .step, .igs
	 * and .brep are supported.
	 */
	...
	public static TopoDS_Vertex getVertex(TopoDS_Shape shape, int id) {
		TopExp_Explorer exp = new TopExp_Explorer(shape,
	...
	/**
	 * Compute the tolerance of shapes by selecting the highest tolerance of the
	 * give child shapes
	 */
